...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'login') {
      ...
      // On success, App.jsx's onAuthStateChange fires → no navigation needed here
    } else {
      ...
      if (err) {
        ...
        // Update the auto-created profile row with the user's name
        await supabase.from('profiles').update({ name: name.trim() }).eq('id', data.user.id);
        // If email confirmation is disabled (recommended for company-internal apps),
        // the onAuthStateChange event fires immediately. If enabled, show a message.
        if (data.session) {
          // Already logged in — App.jsx will handle the redirect
        } else {
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <div className="app-logo" style={{ fontSize: '24px', marginBottom: '6px' }}>
            Fund<span>Lens</span>
          </div>
        ...
        <div className="card fade-in" style={{ padding: '32px' }}>
          ...
          <p style={{ fontSize: '12px', color: 'var(--text3)', marginBottom: '24px' }}>
            {mode === 'login'
              ? 'Your analysis, your history, your settings — always here.'
              : 'Set up your profile once. Run analysis anytime.'}
          </p>

          {error && (
            <div style={{
              background: 'var(--red-bg)', border: '1px solid var(--red-bd)',
              color: 'var(--red)', borderRadius: '7px', padding: '10px 12px',
          ...
          {info && (
            <div style={{
              ...
              color: 'var(--green)', borderRadius: '7px', padding: '10px 12px',
              fontSize: '12px', marginBottom: '16px',
            }}>
              {info}
            </div>
        ...
        <p style={{ textAlign: 'center', fontSize: '11px', color: 'var(--text3)', marginTop: '20px' }}>
          FundLens is a private 401K analysis tool.
        </p>
      </div>
    </div>
  );
