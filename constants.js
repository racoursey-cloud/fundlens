// ═══════════════════════════════════════════════════════════════
// FundLens v4 — Engine Constants
// Ported verbatim from v3 index.html. Do not modify scoring values
// without updating SEED calibration notes below.
// ═══════════════════════════════════════════════════════════════

// TA department's 22-fund 401K universe.
// Only used by COMPANY_CODES['TA26'] and as the SEED/ALERTS reference set.
// New users start with an empty fund list — the wizard populates it.
...
// Key FRED series for macro intelligence (fetched sequentially — parallel overwhelms proxy)
...
// Money market funds file N-MFP, not N-PORT-P — no holdings to fetch from EDGAR
export const MONEY_MARKET_FUNDS = new Set(['FDRXX', 'ADAXX']);

// Company codes — maps short code entered in wizard to a named fund universe.
// To add a new company: COMPANY_CODES['XYZ26'] = { name: 'Company Name', funds: [...] }
...
// TA department fund-specific research notes. Shown in Investment Case risk section.
// Non-TA users: ALERTS[ticker] returns undefined, handled gracefully throughout.
export const ALERTS = {
...
// SEED — fallback composite scores shown before first live run.
// Calibrated to 'Sell America' macro regime of Q1 2026.
// For non-TA tickers: SEED[ticker] is undefined → composite:5 fallback.
export const SEED = {
...
// 4-Factor Scoring Model — default weights (sum to 100)
// composite = mandateScore×W₁ + momentum×W₂ + riskAdj×W₃ + managerQuality×W₄ − concentrationPenalty
...
// Plain-English display names for the 4 scoring factors
export const FACTOR_LABELS = {
  mandateScore:   { label: 'Macro Fit',    desc: "How well this fund's investment mandate aligns with current macro conditions — scored directly by AI" },
...
// World data freshness: minutes before FRED+GDELT+sector scoring is considered stale.
// Steps 1 and 3 are skipped when cache is still valid.
export const DEFAULT_WORLD_TTL_MINS = 60;

// RSS news feeds (CORS-proxied through /api/rss)
...
// Modified Z-score tiers (robust to small n=20-25 samples)
// modZ = 0.6745 × (composite − median) / MAD
export const TIERS = {
  BREAKAWAY: { min: 2.0,  label: 'BREAKAWAY', cls: 't1' },
  STRONG:    { min: 1.2,  label: 'STRONG',    cls: 't2' },
  SOLID:     { min: 0.3,  label: 'SOLID',     cls: 't3' },
  NEUTRAL:   { min: -0.5, label: 'NEUTRAL',   cls: '' },
  WEAK:      { min: -Infinity, label: 'WEAK', cls: 't4' },
};
